...
public class Utils {
     ...
     * then method in superinterface will be considered as "implemented method", instead of "overridden".
     *
     * @return a list of implemented methods
     */
    public List<Element> getImplementedMethods(Element method, List<Element> implementedMethods) {
        TypeElement encl = getEnclosingTypeElement(method);
        if (encl == null) {
            return implementedMethods;
        }

        for (TypeElement interfaceType : getImplementedInterfaces(encl)) {
            Element implementedMethod = getMemberBySignature(interfaceType, ElementKind.METHOD, method.toString());
            if (implementedMethod != null) {
                implementedMethods.add(implementedMethod);
                getImplementedMethods(implementedMethod, implementedMethods);
            }
        }
        return implementedMethods;
    }
     ...
     * This is used for search inherited inline tags.
     * Follow the same as Standard doclet,
     * search in the order of appearance following the word implements in declaration.
     *
